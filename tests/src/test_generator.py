...
class TestGenerator:
    ...
    async def generate_skeleton(
        ...
        """Generate placeholder-based skeleton code with strict "no guess" validation.

        Args:
            user_story: The user story text.
            conditions: The acceptance criteria text.
            target_urls: Optional list of target URLs.
        ...
        prompt = get_skeleton_prompt_template(
        ...
        logger.info(
            "Skeleton prompt generated: %d chars, expected_count=%s, has_count_header=%s",
            len(prompt),
            expected_count,
            "EXACTLY" in prompt and "SEPARATE test functions" in prompt,
        )
        ...
        for attempt in range(max_retries):
            ...
            if not code or not code.strip():
                logger.warning("Skeleton LLM returned empty response (attempt %d/%d)", attempt + 1, max_retries)
                continue

            logger.info(
                "Skeleton LLM response: %d chars, test functions found: %d",
                len(code),
                len(self.parser.test_definition_pattern.findall(code)),
            )

            validation_error = self.parser.validate_skeleton(code)
            if not validation_error:
                return code

            logger.warning(
                "Skeleton validation failed (attempt %d/%d): %s",
                attempt + 1,
                max_retries,
                validation_error,
            )
            ...
            prompt += f"\n\nERROR FROM LAST ATTEMPT: {validation_error}\nPlease try again and strictly follow the NEVER GUESS LOCATORS rule."

        raise ValueError(f"Failed to generate valid skeleton after {max_retries} attempts: {last_error}")
