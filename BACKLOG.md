...
**Fix in progress (2026-07-23):** plan of record `docs/plans/AI-052_observed_transitions_plan.md` — 6 sessions. **S1 ✅** (capture `ObservedTrail` in `JourneyScraper`) + **S2 ✅** (plumbed into resolver) + **S3 ✅** (core fix: trail-driven scoping, strict no-all-pages-fallback, divergence-aware replay of proven selectors, divergence latch; 19 unit tests; both verify sites show **zero different-page locator errors** — see `docs/sessions/2026-07-23_ai052_session3_core_fix.md`). Committed. **S4 (delete keyword-URL guessing)** next. Remaining saucedemo failure is AI-051 (out of scope). Status stays `🆕` until S6 ship (status flips only at ship-it, per housekeeping rule).
