...
**Status:** 🟡 ready-for-agent → **FIX IMPLEMENTED 2026-08-29** (committed `288e2f8`): `PlaceholderScorer._container_aggregate_penalty` — (a) a generic container (`main`/`body`/`div`/`section`/`nav`/`header`/`footer`/`article`/`form`) that matched only via MERGED descendant text gets −40 on the haystack fast path (skipped for page-level ASSERTs via `PAGE_LEVEL_ASSERT_TERMS`; only tag/role-verified containers); (b) for CLICK, a non-interactive **prose text-run** (`p`/`h1-6`/`li` with no link/button role/href) also gets −40 so a real interactive target always wins. **Seeded-store A/B DONE (2026-08-29, uncommitted)** — `scripts/ai058_seeded_ab.py`: hermetic driver (temp dirs, own `AITEST_STORAGE_ROOT`, auto-learn off) that seeds ONE known negative (the recurring banking `#payment-error` for ASSERT 'payment success message') and verifies it round-trips: insert → `find_negative` row → step-scoped score −4 on `#payment-error` / 0 on `#payment-success-title`. **Leak-audited:** only runtime artifacts land in the temp dir; `mock_sites/banking` restored (git checkout, clean), `baseline.json`/`eval_results.db`/`generated_tests/` never touched. Deterministic generation-level A/B (control vs seeded treatment) produced IDENTICAL locators (`#welcome-message`) this run — the resolver's element pick is LLM-variance dominated, so a generation A/B cannot force the exact wrong-pick scenario; the SCORING-level flip (this seed test) is the strongest deterministic proof. 2906 pytest (+1), ruff + mypy clean, eval static 97.9%.
**Resolver-level A/B DONE (2026-08-29, uncommitted)** — `scripts/ai058_resolver_ab.py` + unit test `test_resolver_ab_downweights_wrong_pick_on_own_step`: fully deterministic (frozen `scraped_pages/` pool, no LLM, no mock server). Leg 1 (payments-only, the historical test_09 context): control ranks the wrong hidden `#payment-error` first (52); the seeded negative drops it to 32. Leg 2 (consolidated pool): the correct `#payment-success-message` (112) already wins post-AI-064 and the negative cannot hurt it. Leg 3 (step-scoping guard): a different step ('payee') is byte-identical with/without the seed. This is the strongest deterministic proof of the step-scoped flip — it reproduces exactly the real evidence (wrong pick wins in the payments-only context, correct element wins when both pages are scraped) with zero run-to-run variance.
