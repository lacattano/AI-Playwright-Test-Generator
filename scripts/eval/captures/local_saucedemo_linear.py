...
def test_01_log_in_with_credentials(page: Page, evidence_tracker):
    ...
    evidence_tracker.assert_visible('[data-test="inventory-item"]', label='product list visible')
...
def test_02_add_item_to_cart(page: Page, evidence_tracker):
    ...
    evidence_tracker.fill('#password', 'secret_sauce', label='password')
    evidence_tracker.click('#login-button', label='login button')
    inventory_page.click('backpack add to cart')
...
def test_03_navigate_to_shopping_cart(page: Page, evidence_tracker):
    home_page = HomePage(page, evidence_tracker)
    inventory_page = InventoryPage(page, evidence_tracker)
    home_page = HomePage(page, evidence_tracker)
    evidence_tracker.navigate('https://www.saucedemo.com')
...
def test_04_verify_item_in_cart(page: Page, evidence_tracker):
    ...
    evidence_tracker.click('#login-button', label='login button')
    inventory_page.click('backpack add to cart')
    evidence_tracker.assert_visible('.shopping_cart_badge[data-test="shopping-cart-badge"]', label='backpack item in cart')
...
def test_05_navigate_to_checkout(page: Page, evidence_tracker):
    ...
    evidence_tracker.click('#login-button', label='login button')
    inventory_page.click('backpack add to cart')
    evidence_tracker.assert_visible('[data-test="title"]', label='checkout form visible')

@pytest.mark.evidence(condition_ref="TC-06", story_ref="S01")
def test_06_complete_checkout_success(page: Page, evidence_tracker):
    home_page = HomePage(page, evidence_tracker)
    inventory_page = InventoryPage(page, evidence_tracker)
    home_page = HomePage(page, evidence_tracker)
    evidence_tracker.navigate('https://www.saucedemo.com')
    ...
    evidence_tracker.assert_visible('[data-test="title"]', label='thank you message')