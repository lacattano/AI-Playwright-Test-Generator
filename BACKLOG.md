...
**Remaining:** `learn: true` input exists but fails fast (store-caching learning arrives after Phase 7). **Real GitLab.com gate: ✅ passed 2026-08-15** — `scripts/ci_gitlab_real_project_test.py` against `cat-tan-operations/ai-testgen-selftest` (14/14 live checks: push pipeline + junit 8/8 + cache miss, MR §6 note posted + edited-not-duplicated, cache hit on re-run; account needed GitLab identity verification first).
