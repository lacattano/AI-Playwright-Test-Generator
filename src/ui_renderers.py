...
class SavedPackagePanel:
    ...
    Discovers previously generated test packages, displays their metadata,
    run history, and flakiness metrics. Allows re-running saved suites.
    """
    ...
    def render_sidebar(self) -> None:
        """Render the sidebar section for loading saved packages."""
        ...
        # Show loaded package summary if a package is already loaded
        loaded = st.session_state.get("loaded_package_manifest")
        if loaded:
            self._render_loaded_summary()

    def _render_loaded_summary(self) -> None:
        """Render summary of the currently loaded package in the sidebar."""
        loaded = st.session_state.get("loaded_package_manifest")
        if not loaded:
            return
        ...
        if history:
            st.sidebar.metric(
                "Total Runs",
                history.get("total_runs", 0),
            )
            st.sidebar.metric(
                "Total Passed",
                history.get("total_passed", 0),
            )
            st.sidebar.metric(
                "Total Failed",
                history.get("total_failed", 0),
            )
    ...
    def render_main_panel(self) -> bool:
        ...
        Returns:
            True if a package is loaded and was rendered, False otherwise.
        """
        loaded = st.session_state.get("loaded_package_manifest")
        if not loaded:
            return False
    ...
    def _render_flaky_tests(self, flaky: list[dict]) -> None:
        """Render flaky tests warning section."""
    ...
    def _handle_rerun_saved_suite(self, package_root: str) -> None:
        """Handle re-running a saved test suite from the UI."""
        from src.pipeline_run_service import PipelineRunService
    ...
    def _handle_rerun_failed_only(self, package_root: str, previous_run: Any | None) -> None:
        """Handle re-running only failed tests from a saved suite."""
        from src.pipeline_run_service import PipelineRunService

        st.session_state.pipeline_saved_path = package_root
        # Convert persisted run to RunResult for pipeline service
        previous = None
    ...
    def _load_previous_run(self, package_root: str) -> Any | None:
        """Load the most recent run result for a package."""
