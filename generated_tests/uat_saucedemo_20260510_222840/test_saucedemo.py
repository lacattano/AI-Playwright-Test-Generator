...
def test_03_navigate_to_cart(page, evidence_tracker):
    ...
    evidence_tracker.assert_visible('#continue-shopping', label='shopping cart page')

@pytest.mark.evidence(condition_ref="TC-04", story_ref="S01")
def test_04_verify_item_in_cart(page, evidence_tracker):
    pytest.skip("Skipping: unresolved placeholders for: 'add to cart button for Sauce Labs Backpack'; 'Sauce Labs Backpack listed in cart'")
    evidence_tracker.navigate('https://www.saucedemo.com')
    dismiss_consent_overlays(page)
    evidence_tracker.fill('#user-name', 'standard_user', label='username input')
    evidence_tracker.fill('#password', 'secret_sauce', label='password input')
    ...
    pytest.skip("Unresolved placeholder: {{CLICK:add to cart button for Sauce Labs Backpack}}")
    evidence_tracker.click('#add-to-cart-test.allthethings\\(\\)-t-shirt-\\(red\\)', label='shopping cart icon')
    pytest.skip("Unresolved placeholder: {{ASSERT:Sauce Labs Backpack listed in cart}}")
...
def test_06_complete_checkout(page, evidence_tracker):
    ...
    evidence_tracker.click('#add-to-cart-test.allthethings\\(\\)-t-shirt-\\(red\\)', label='shopping cart icon')
    evidence_tracker.click('#checkout', label='checkout button')
    evidence_tracker.fill('#first-name', 'John', label='first name input')
    evidence_tracker.fill('#last-name', 'Doe', label='last name input')
    evidence_tracker.fill('#postal-code', '12345', label='zip code input')
