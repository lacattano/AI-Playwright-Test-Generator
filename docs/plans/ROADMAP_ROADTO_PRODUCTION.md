...
**Status:** `[~]` **Full spec complete 2026-09-01** — decisions D1–D12 + four refinements locked; implementation not started. **Spec (canonical):** `docs/specs/FEATURE_SPEC_test_to_document_traceability.md` (supersedes the 2026-08-25 `SESSION_SPEC` notes). This item is the phase tracker.  
...
**Phases:**
- [ ] **Phase 1 — Stop discarding provenance** (page + route on chunks; identity through retrieval). ~1 session
- [ ] **Phase 2 — Whole-document generation** (merged with AI-055's remaining per-page OCR wiring; removes the 500-char ceiling; needs a scanned-PDF test fixture). ~1–2 sessions
- [ ] **Phase 3 — Citations per criterion** (hybrid mechanism, verification, unresolved handling). ~2 sessions
- [ ] **Phase 4 — Surfaces** (test-file `# Source:` comments, Living Test Plan cards, CLI debug, `PRIVACY_MODE` exports; optional click-through / AI-028 tie-in). ~1–2 sessions
