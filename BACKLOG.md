...
**Status:** 🆕 new (OPEN — model question still unanswered). **2026-08-18: original conclusion INVALID as stated** (temp-unset + server-config drift + resolution-timeout noise). **2026-08-19: infra fixes shipped (AI-048/049/050) + first clean single-config A/B (spec OFF, thinking OFF): 3.6 = 76.1% vs 3.8 = 62.4% resolution.** BUT this is NOT a valid model verdict — Qwen's own model card shows 3.8 beating 3.6 by 8–20pts (WebArena 64.8 vs 48.8, SWE-bench Pro 61.7 vs 53.5) **with thinking ON**, and our A/B ran both with thinking OFF (removing 3.8's documented advantage). **Re-answer REQUIRES:** (1) external-benchmark validation (lm-eval: GPQA/LiveCodeBench on both GGUFs, thinking ON), (2) thinking-ON project A/B with adequate reasoning token budget. Full record + handover: `docs/sessions/2026-08-19_thinking_collapse_and_ab.md` §5–§8.
